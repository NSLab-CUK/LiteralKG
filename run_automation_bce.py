...
def run_finetuning(filename, index, device):
    ...
    cmd = f"python main_finetuning_BCE.py --aggregation_type {data['Aggregator'][index]} --n_conv_layers {data['Number Layers'][index]} --lr {data['Learning Rate'][index]} --mess_dropout {data['Dropout'][index]} --conv_dim {data['Convolutional Dim'][index]} --pre_training_batch_size {data['Batch Size'][index]} --fine_tuning_batch_size {data['Batch Size'][index]} --pretrain_epoch {int(data['Best Pretrain'][index])} --evaluation_row {index} --device {device} --evaluation_file {filename}"
...
def run_testing(filename, index, device):
    ...
    cmd = f"python test_BCE.py --aggregation_type {data['Aggregator'][index]} --n_conv_layers {data['Number Layers'][index]} --lr {data['Learning Rate'][index]} --mess_dropout {data['Dropout'][index]} --conv_dim {data['Convolutional Dim'][index]} --pre_training_batch_size {data['Batch Size'][index]} --fine_tuning_batch_size {data['Batch Size'][index]} --finetune_model_epoch {data['Best Finetune'][index]} --evaluation_row {index} --device {device} --evaluation_file {filename}"
